...
mode: agent
tools:
  - gisblox-mcp/postalcodes_pc4_geometry_list
  - gisblox-mcp/postalcodes_pc6_geometry_list
  - gisblox-mcp/visualize_pc4_get
  - gisblox-mcp/visualize_pc6_get
---
...
Either use the `gisblox-mcp/postalcodes_pc4_geometry_list` or `gisblox-mcp/postalcodes_pc6_geometry_list` tool to fetch the relevant postal code data.
...
- Wijk (district)
- Buurt (neighborhood) - only if 6 digit postal codes are requested

After you have processed this data, call the `gisblox-mcp/visualize_pc4_get` or `gisblox-mcp/visualize_pc6_get` tool to generate a geojson.io URL for each postal code.
